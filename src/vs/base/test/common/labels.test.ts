...
suite('Labels', () => {
	...
	test('template', () => {
		...
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: '', appName: 'VSCodius', separator: { label: ' - ' } }), 'VSCodius');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'Untitled-1', rootName: '', appName: 'VSCodius', separator: { label: ' - ' } }), 'Untitled-1 - VSCodius');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco', appName: 'VSCodius', separator: { label: ' - ' } }), 'monaco - VSCodius');
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'VSCodius', separator: { label: ' - ' } }), 'somefile.txt - monaco - VSCodius');
		assert.strictEqual(labels.template(t, { dirty: '* ', activeEditorShort: 'somefile.txt', rootName: 'monaco', appName: 'VSCodius', separator: { label: ' - ' } }), '* somefile.txt - monaco - VSCodius');

		// real world example (other)
		t = '${dirty}${activeEditorShort}${separator}${rootNameShort}${separator}${appName}';
		assert.strictEqual(labels.template(t, { dirty: '', activeEditorShort: '', rootName: 'monaco (Workspace)', rootNameShort: 'monaco', appName: 'VSCodius', separator: { label: ' - ' } }), 'monaco - VSCodius');
	});
