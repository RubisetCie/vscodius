...
export class ExtensionManagementService extends AbstractExtensionManagementService implements INativeServerExtensionManagementService {
	...
	async download(extension: IGalleryExtension, operation: InstallOperation): Promise<URI> {
		const location = await this.downloadExtension(extension, operation);
		return location;
	}
	...
	private async downloadAndExtractGalleryExtension(extensionKey: ExtensionKey, gallery: IGalleryExtension, operation: InstallOperation, options: InstallExtensionTaskOptions, token: CancellationToken): Promise<ILocalExtension> {
		const location = await this.downloadExtension(gallery, operation, options.context?.[EXTENSION_INSTALL_CLIENT_TARGET_PLATFORM_CONTEXT]);
		try {
	...
	private async downloadExtension(extension: IGalleryExtension, operation: InstallOperation, clientTargetPlatform?: TargetPlatform): Promise<URI> {
		const location = await this.extensionsDownloader.download(extension, operation, clientTargetPlatform);
