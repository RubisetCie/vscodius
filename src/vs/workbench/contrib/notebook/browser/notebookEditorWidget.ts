...
export class NotebookEditorWidget extends Disposable implements INotebookEditorDelegate, INotebookEditor {
	...
	async setModel(textModel: NotebookTextModel, viewState: INotebookEditorViewState | undefined, viewType?: string): Promise<void> {
		if (this.viewModel === undefined || !this.viewModel.equal(textModel)) {
			...
			this._detachModel();
			await this._attachModel(textModel, viewType ?? textModel.viewType, viewState);
	...
	private async _attachModel(textModel: NotebookTextModel, viewType: string, viewState: INotebookEditorViewState | undefined) {
