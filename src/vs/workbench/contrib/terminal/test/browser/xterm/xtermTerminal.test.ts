...
suite('XtermTerminal', () => {
	...
	setup(async () => {
		...
		xterm = store.add(instantiationService.createInstance(TestXtermTerminal, XTermBaseCtor, 80, 30, { getBackgroundColor: () => undefined }, capabilityStore, ''));
	...
	suite('theme', () => {
		test('should apply correct background color based on getBackgroundColor', () => {
			...
			xterm = store.add(instantiationService.createInstance(XtermTerminal, XTermBaseCtor, 80, 30, { getBackgroundColor: () => new Color(new RGBA(255, 0, 0)) }, store.add(new TerminalCapabilityStore()), ''));
		...
		test('should react to and apply theme changes', () => {
			...
			xterm = store.add(instantiationService.createInstance(XtermTerminal, XTermBaseCtor, 80, 30, { getBackgroundColor: () => undefined }, store.add(new TerminalCapabilityStore()), ''));
